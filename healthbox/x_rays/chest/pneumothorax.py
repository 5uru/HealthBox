...
class PneumothoraxDetection(Model):
    def __init__(self):
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        model = AutoModel.from_pretrained("ianpan/pneumothorax-cxr",
    ...
    def predict(self, image) -> dict:
        ...
        if ptx_score > 0.5:
            info_string = f":red[Pneumothorax: {ptx_score}]"
        else:
            info_string = f":blue[Pneumothorax: {ptx_score}]"
